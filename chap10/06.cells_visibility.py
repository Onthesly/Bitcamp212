...
train_image = cv2.imread('train_numbers.png', cv2.IMREAD_COLOR)
...
cells = [np.hsplit(row,nsample) for row in np.vsplit(train_image,nclass)]   # numpy 함수 사용
cells = np.reshape(cells, (-1, 40, 40, 3))
